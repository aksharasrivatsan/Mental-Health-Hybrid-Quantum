...
FINE_TUNED_VGG_WEIGHTS = (
    Path("models") / "vgg16_block5_fine_tuned.weights.h5"
)
...
def build_hybrid_vgg_quantum_model(
    *,
    ...
    if not FINE_TUNED_VGG_WEIGHTS.exists():
        raise FileNotFoundError(
            f"Fine-tuned VGG16 weights not found at "
            f"{FINE_TUNED_VGG_WEIGHTS}. "
            "Run VGG16_fine_tuned.py first."
        )
    ...
    # Load the VGG16 weights produced by Stage 1.
    base_model.load_weights(
        FINE_TUNED_VGG_WEIGHTS
    )

    # ------------------------------------------------------------
    # Freeze Blocks 1–4 and fine-tune Block 5
    # ------------------------------------------------------------
